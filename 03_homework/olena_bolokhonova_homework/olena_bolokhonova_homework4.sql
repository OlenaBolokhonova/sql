...
-- Query to number each customer's unique market dates using DENSE_RANK()
SELECT 
    customer_id,
    market_date,
...
-- Create a CTE to find sales values grouped by dates
...
-- Create another CTE with row numbers for both highest and lowest sales days
ranked_sales AS (
    SELECT 
        market_date,
        total_sales_value,
        ROW_NUMBER() OVER (ORDER BY total_sales_value DESC) AS rn_max,
        ROW_NUMBER() OVER (ORDER BY total_sales_value ASC) AS rn_min
    FROM 
        sales_by_date
)

-- Query the second CTE twice, once for the highest sales day and once for the lowest sales day, then combine them using UNION
SELECT 
    market_date,
    total_sales_value,
